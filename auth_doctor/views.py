...
def doc_signin(request):
    ...
    if request.method == "POST":
        ...
        if user is not None:
            ...
            return render(request,'doctor/doc_index.html', {'fname':fname})

        else:
            messages.error(request, "Wrong Username/password")

    return render(request, "authentication_doctor/doctor_login.html")
...
def doc_register(request):
    ...
    return render(request, "authentication_doctor/doctor_signup.html")
...
def activate(request, uidb64, token):
    ...
    if myuser is not None and generate_token.check_token(myuser, token):
        ...
        return redirect('/doctor_signin', context)
    else:
